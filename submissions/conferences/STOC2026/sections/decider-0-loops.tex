...
Proving the correctness of this decider is surprisingly nontrivial. Let us represent the space-time diagram of a given Turing machine $\mathcal{M}$ (Appendix~\ref{sec:TMs}) using partially defined functions $f_\mathcal{M},g_\mathcal{M},h_\mathcal{M}$, and, $F_\mathcal{M}$:
\begin{align*}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item Similarly to above, $F(p_{l-1}) = F(p'_{l-1})$ implies that the machine will transition to the same state after $p_{l-1}$ and $p'_{l-1}$, giving $h(t^*) = h(t'_0)$.
...
\input{sections/decider-0-loop-results}