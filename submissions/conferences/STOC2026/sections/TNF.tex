...
\begin{figure}[ht]
    \centering
    \resizebox{0.7\textwidth}{!}{ % Rescale to fit horizontally
        \begin{tikzpicture}[
            ...
            ($(root.north west) + (1cm,-1.35cm)$)
            rectangle
            ($(root.north west) + (1.74cm,-0.95cm)$);

            % Highlight the B0 transition in the child3 table
            \draw[magenta, thick, line width=0.8mm]
            ($(child3.north west) + (1.1cm,-1.85cm)$)
            rectangle
            ($(child3.north west) + (2cm,-1.45cm)$);

            % Highlight the B0 transition in the child4 table
            \draw[magenta, thick, line width=0.8mm]
            ($(child4.north west) + (1.1cm,-1.85cm)$)
            rectangle
            ($(child4.north west) + (2cm,-1.45cm)$);
...
Syntactically, as defined in Appendix~\ref{sec:TMs}, there are $(2ns + 1)^{ns}$ Turing machines with $n$ states and $s$ symbols. This gives $21^{10} \simeq 1.67\times10^{13} \simeq 16.7 \text{ trillion}$ possible 5-state 2-symbol Turing machines. However, naively counting Turing machines this way does not account for two phenomena:
\begin{enumerate}
