...
The $n$-gram Closed Position Set (\ngramcps) decider which we introduce here is a simplification of an earlier technique introduced in \texttt{bbfind} \cite{Skelet_bbfind}, Closed Position Set (CPS), see Section~\ref{sec:intro:mainresults}. \ngramcps is both relatively simple and potent, as it decides 99.89\% of all nonhalting enumerated 5-state machines excluding loops, see Table~\ref{tab:pipelineBB5}. The method is strongest when augmenting the binary alphabet of Turing machines with extra information on the tape, such as a fixed-length history of previously seen (state,symbol) pairs, see Section~\ref{sec:n-gramCPS:augmentations}.
