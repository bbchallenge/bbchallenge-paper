...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \State \For{$i$ \textbf{in} $[0,l+o[$ } \Comment{Note: $o$, and therefore also the loop bounds, vary between iterations}
        \If{$K-i < 0$} \Comment{If we have reached the beginning of the transcript, try another $l$}
        \State \textbf{break}
        \State \EndIf
        \State $s,m,d = T_{L-i}$
        \State $s',m',d' = T_{K-i}$
