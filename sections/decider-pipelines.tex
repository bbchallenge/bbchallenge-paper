...
\begin{table}[h!]
    ...
    \caption{Approximation of the $S(5)$ pipeline as implemented in \CoqBB. All the $\BBtheFifthTNF$ enumerated 5-state machines are decided by this pipeline, which solves $S(5) = \BBtheFifth$, see Theorem~\ref{th:BB5}. The exact pipeline, with decider parameters, is given in Appendix~\ref{app:pipelines}. }
...
\begin{table}[h!]
    ...
    \end{tabular}
    \caption{Approximation of the $S(2,4)$ pipeline as implemented in \CoqBB. All the $\BBTxFTNF$ enumerated 2-state 4-symbol machines are decided by this pipeline, which solves $S(2,4) = \BBTxF$, see Theorem~\ref{th:BB2x4}. The exact pipeline, with decider parameters, is given in Appendix~\ref{app:pipelines}. }\label{tab:pipelineBB2x4}
\end{table}
...
\begin{table}[h!]
    ...
    \begin{tabular}{|l|rrr|}
        ...
        \hline
    \end{tabular}
    \caption{Approximation of the $S(4)$ pipeline as implemented in \CoqBB. All the $\BBtheFourthTNF$ enumerated 4-state machines are decided by this pipeline, which brings confirmation that $S(4) = \BBtheFourth$ \cite{Brady83}, see Theorem~\ref{th:BB4}. The exact pipeline, with decider parameters, is given in Appendix~\ref{app:pipelines}. }\label{tab:pipelineBB4}
\end{table}
...
All these deciders can be expressed in the same general framework known as Closed Tape Language (CTL), which is attributed to Marxen and Buntrock and first documented by Ligocki \cite{ShawnCTL}:
...
\paragraph{Deciders vs. verifiers.} While we include all methods under the ``decider" umbrella, in \CoqBB, only the \textit{verifier} part of FAR and WFAR are implemented. This means that instead of searching for a CTL set $C$ (see above), it is given and verified correct: \CoqBB hardcodes a total of $40$ FAR /
...
\paragraph{\texttt{1RB}-reduction.} Any TNF-enumerated machine (Section~\ref{sec:enum}) whose initial $n$ transitions, $n\geq1$, write a \szero and has at least one transition writing a \sone (TNF guarantees the transition is reachable) can be transformed into a machine that starts by writing a \sone and visits all the same configurations (up to state-renaming) except the first $n$, see \CoqBB's function \texttt{TM\_to\_NF}. We call this transformation \texttt{1RB}-reduction, as \texttt{1RB} is the first transition of the new machine. Hence, any machine whose reduction to \texttt{1RB} already has a proof of nonhalting can be decided using the same argument! This is proven in \CoqBB's \texttt{Lemma TM\_to\_NF\_spec}. For instance, TNF-enumerated machine \texttt{0RB0LD\_1RC1RE\_1LA1RC\_1LC1LD\_---0RB} \texttt{1RB}-reduced to sporadic machine ``Finned \#3'' (Section~\ref{sec:sporadic}), so is decided using the same proof. In the $S(5)$ pipeline (Table~\ref{tab:pipelineBB5}) this argument is used to decide 24 machines whose \texttt{1RB}-reduction correspond to 23 FAR/WFAR certificates (see above) and one sporadic machine, ``Finned \#3''.
