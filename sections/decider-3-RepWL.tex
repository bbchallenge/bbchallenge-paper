...
The Repeated Word List (RepWL) technique, introduced by mxdys in \CoqBB, is based on the following simple idea: if a word (or \textit{block}) of length $l > 0$ appears consecutively on the tape more than $T > 0$ times (with $l, T \in \mathbb{N}$ fixed) then we assume it may repeat an unbounded number of times in the future. In practice, it means we represent configurations as regular expressions.

Consider the following configuration in \textit{directional head notation}, where the head lives in between cells and points either right or left. This framework is equivalent to the Turing machines setup used elsewhere in this work, see Section~\ref{sec:TMs}.

$$ \texttt{0}^\infty \; \texttt{11100 A> 11110101010111111111} \; \texttt{0}^\infty$$

\noindent Using $l=2$ and $T = 3$, we can represent the configuration as

$$ \texttt{0}^\infty \; (\texttt{01}) \; (\texttt{11}) \; (\texttt{00}) \; \texttt{A>} \; (\texttt{11})^2 \; (\texttt{01})^{3+} \; (\texttt{11})^{3+} \; \texttt{0}^\infty $$

\noindent where blocks are constructed from the head outwards, with symbols \szero drawn from $0^\infty$ if the number of symbols on either side of the head is not a multiple of $l$. Any repetition of more than $T$ times the same word $w \in \{\szero,\sone\}^l$ is replaced by the regular expression $(w)^{T+}$ meaning that word $w$ is repeated at least $T$ times. We call $T$ the block-repeat threshold. 
